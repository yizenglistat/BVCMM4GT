...
update_beta <- function(center=FALSE, Xd, omega=1, sigma=1, 
	...
	beta_d_config)
{
	...
	proposed_one 						<-  -determinant(proposed_R_knots_mat)$modulus/2-updated_tau*crossprod(beta_d_knots, proposed_R_knots_inv_mat)%*%beta_d_knots/2-sum(proposed_h_minus_eta*omega*proposed_h_minus_eta)/(2*sigma**2)
	current_one 						<-  -determinant(R_knots_mat)$modulus/2-updated_tau*crossprod(beta_d_knots, R_knots_inv_mat)%*%beta_d_knots/2-sum(h_minus_eta*omega*h_minus_eta)/(2*sigma**2)
